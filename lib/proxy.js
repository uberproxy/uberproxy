...
Proxy.prototype.find_worker = function(host, workerAddr) {
    if (this._hexpr.exec(host)) {
        for (var i in this._rules) {
           if (!this._rules.hasOwnProperty(i)) continue;
            if (this._rules[i].exec(host)) {
                for (var rnd = 0; rnd < this._apps[i].length; rnd++) {
                    if (!this._apps[i].hasOwnProperty(rnd)) continue;
                    if (this._apps[i][rnd].address == workerAddr || this._apps[i][rnd]._workerAddr == workerAddr) {
                        return this._apps[i][rnd];
                    }
                }
            }
        }
    ...
    throw new Error("Cannot find worker " + workerAddr + " for application " + host);
};

Proxy.prototype.deregister = function(app) {
    if (!(app instanceof Worker)) throw new Error("Unexpected argument, expecting an Worker object");
