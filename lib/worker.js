...
{
    ['client', 'hostname'].forEach(function(val) {
        if (!obj[val]) {
    ...
    this.address   = this._client + ":" + this._port;
...
Worker.prototype.getId = function() {
    return this.appId + "\0" + this._client;
};
...
Worker.prototype.forward = function(conn) {
    conn.worker = this;
    process.Proxy.emit('preforward', conn);
    if (conn.done) return;
    this.emit('preforward', conn);
    if (conn.done) return;
    conn.slotid = --this._maxreq;
    conn.forwardTo(this);
};
